...
Para calcular la FFT de un conjunto de datos unidimensional use la función SIGNALS::FFT::fft(signal, N, transform), donde:
...
Para ver el resultado puede usar la función SIGNALS::FFT::showTransform(transform,n)

Ejemplo1:
	double signal[4] = {2.0, -9.6, -3.5, 0.6};
	std::complex<double> transform[4];
	SIGNALS::FFT::fft(signal, 4, transform);
	SIGNALS::FFT::showTransform(transform, 4);
...
También se puede hacer lo mismo para datos float o enteros.
